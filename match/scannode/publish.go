...
type PubMsg struct {
	payload  string
	attrs    []*PubMsgAttr
	attrsMap map[string]*PubMsgAttr
}
...
func createPubMsg(attrs map[string]interface{}, payload string) *PubMsg {
	pmsg := &PubMsg{
		...
		attrsMap: make(map[string]*PubMsgAttr),
	}
		...
		if ab, ok := IdxBase.attrbases[name]; ok {
			...
			if int(ab.use) == dmq.AttrUseField["range"] {
				if f, ok := attr.(float64); ok {
					pmsgAttr := &PubMsgAttr{name: name, val: f}
					pmsg.attrs = append(
						pmsg.attrs, pmsgAttr)
					pmsg.attrsMap[name] = pmsgAttr
				}
			}
		}
	}
...
func processPushMsg(msg *DecodedMsg) error {
	...
	pcgroupMap := findCandSubClis(pubmsg)
...
func msgMatch(scInfo *SubCliInfo, pubmsg *PubMsg) bool {
	for _, attr := range scInfo.Attrs {
		if mattr, ok := pubmsg.attrsMap[attr.name]; ok {
			if int(attr.use) == dmq.AttrUseField[dmq.AttrUseRange] {
				if attr.low > mattr.val || attr.high < mattr.val {
					return false
				}
			}
		}
...
func findCandSubClis(pubmsg *PubMsg) map[string]*PubCliGroup {
	// Group subclients by their belonging datanode, mapping key is the Id of datanode
	pcgroupMap := make(map[string]*PubCliGroup)

	for _, scInfo := range ClisInfo {
		scInfo.lock.RLock()
		if msgMatch(scInfo, pubmsg) {
			vn := Rtable.StoreSearch(scInfo.CidHash)
			if vn == nil {
				log.Error("couldn't find vnode for subcli %s",
					hex.EncodeToString(scInfo.Cid))
				continue
			}
